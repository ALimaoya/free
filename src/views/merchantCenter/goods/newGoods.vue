    ...
    <div class="newGoods new goods">
      ...
      <el-form :model="form" ref="form" :rules="formRule" label-position="right">
        ...
        <el-form-item label="品牌商品视频" labelWidth="130px" prop="video">
          <div class="hoveVideo">
            <el-upload class="avatar-uploader uploadvideo" v-model.trim="form.video" :action="videoUrl" :http-request="uploadSectionFile"
...
export default {
  ...
  methods: {
    ...
    confirmBrand() {
      if (
        this.form.brandId !== "" &&
        this.brandData[this.radio * 1].id !== undefined
      ) {
    ...
    getFirstList() {
      ...
      firstList().then(res => {
        if (res.data.status === "000000000") {
          ...
          // }
          if (
            /^[0-9]+$/.test(this.form.firstType * 1) &&
            this.form.firstType !== ""
          ) {
    ...
    getSecondList(type) {
      ...
      secondList(type).then(res => {
        if (res.data.status === "000000000") {
          ...
          // }
          if (
            /^[0-9]+$/.test(this.form.secondType * 1) &&
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            if (this.form.introduce === "") {
              this.form.introduce = this.form.productName;
            }
            let data = {
              id: "",
